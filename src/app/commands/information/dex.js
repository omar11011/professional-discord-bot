...
module.exports = new Command({
    ...
    run: async (client, message, props) => {
        ...
        if (!data || data.error) return message.reply(i18n.res('dex.error', props.lang))
        ...
        if (data.evolutions.level.length > 0) {
            for (let i = 0; i < data.evolutions.level.length; i++) description += `\n${i18n.res('dex.evolutions.level', props.lang, { evolution: data.evolutions.level[i].pokemon, level: data.evolutions.level[i].level })}`
        }
        if (data.evolutions.mega.length > 0) {
            for (let i = 0; i < data.evolutions.mega.length; i++) description += `\n${i18n.res('dex.evolutions.mega', props.lang, { evolution: data.evolutions.mega[i].pokemon, item: data.evolutions.mega[i].item.idioms[props.lang] })}`
        }
        if (data.evolutions.giga.length > 0) {
            for (let i = 0; i < data.evolutions.giga.length; i++) description += `\n${i18n.res('dex.evolutions.giga', props.lang, { evolution: data.evolutions.giga[i].pokemon })}`
        }
